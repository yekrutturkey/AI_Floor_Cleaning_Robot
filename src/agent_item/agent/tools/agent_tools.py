...
from agent_item.rag.rag_service import RagSummarizeService
from agent_item.utils.config_handler import agent_config
...
@tool(description="获取指定城市的天气信息并以字符串的形式返回")
...
def generate_external_data() -> str:
    ...
    if not external_data:
        ...
        if not os.path.exists(external_data_path):
            raise FileNotFoundError(f"外数据文件{external_data_path}不存在")
        ...
        with open(external_data_path, "r", encoding="utf-8") as f:
            for line in f.readlines()[1:]:
                ...
                # 这里子字典是time的原因是因为我们上面自定义的结构里是"month"
